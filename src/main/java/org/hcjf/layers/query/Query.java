...
import org.hcjf.layers.scripting.PythonCodeEvaluator;
...
public class Query extends EvaluatorCollection implements Queryable {
    ...
    static {
        ...
        cache = new LruMap<>(SystemProperties.getInteger(LayerSystemProperties.Query.COMPILER_CACHE_SIZE));
    ...
    public final Query addOrderField(String orderField) {
        return addOrderField(orderField, SystemProperties.getBoolean(LayerSystemProperties.Query.DEFAULT_DESC_ORDER));
    }
    ...
    public final Query addReturnField(String returnField) {
        if(returnField.equals(SystemProperties.get(LayerSystemProperties.Query.ReservedWord.RETURN_ALL))) {
            returnAll = true;
        } else {
    ...
    public final Query addReturnField(QueryReturnParameter returnParameter) {
        if(returnParameter instanceof QueryReturnField && ((QueryReturnField)returnParameter).getFieldPath().equals(
                SystemProperties.get(LayerSystemProperties.Query.ReservedWord.RETURN_ALL))) {
            returnAll = true;
        } else {
     ...
     */
    @Override
    public final <O extends Object> Collection<O> evaluate(Collection<O> dataSource, Consumer<O> consumer) {
     ...
     */
    @Override
    public final <O extends Object> Collection<O> evaluate(DataSource<O> dataSource) {
     ...
     */
    @Override
    public final <O extends Object> Collection<O> evaluate(DataSource<O> dataSource, Consumer<O> consumer) {
        Collection<O> result;
    ...
    public final boolean verifyCondition(Object object) {
        Consumer consumer = new IntrospectionConsumer<>();
    ...
    private boolean isEvaluatorDone(Evaluator evaluator) {
        ...
        if(session != null) {
            List<Evaluator> evaluatorsCache = (List<Evaluator>) session.getProperties().get(
                    SystemProperties.get(LayerSystemProperties.Query.EVALUATORS_CACHE_NAME));
    ...
    private void initializeEvaluatorsCache() {
        ...
        if(session != null) {
            session.put(SystemProperties.get(LayerSystemProperties.Query.EVALUATORS_CACHE_NAME),
                    new ArrayList<Evaluator>());
            session.put(SystemProperties.get(LayerSystemProperties.Query.EVALUATOR_LEFT_VALUES_CACHE_NAME),
                    new HashMap<Evaluator, Object>());
            session.put(SystemProperties.get(LayerSystemProperties.Query.EVALUATOR_RIGHT_VALUES_CACHE_NAME),
                    new HashMap<Evaluator, Object>());
        }
    }
    ...
    private void clearEvaluatorsCache() {
        ServiceSession session = ServiceSession.getCurrentIdentity();
        if(session != null) {
            session.remove(SystemProperties.get(LayerSystemProperties.Query.EVALUATORS_CACHE_NAME));
            session.remove(SystemProperties.get(LayerSystemProperties.Query.EVALUATOR_LEFT_VALUES_CACHE_NAME));
            session.remove(SystemProperties.get(LayerSystemProperties.Query.EVALUATOR_RIGHT_VALUES_CACHE_NAME));
        }
    }
     ...
     */
    private Collection<? extends Joinable> join(DataSource<Joinable> dataSource, Consumer<Joinable> consumer) {
     ...
     */
    private Collection<? extends Joinable> getJoinData(Query query, DataSource<Joinable> dataSource, Consumer<Joinable> consumer) {
    ...
    private Collection<Joinable> product(Collection<? extends Joinable> left, Collection<? extends Joinable> right, Join join,
                                         DataSource<? extends Joinable> dataSource, Consumer<? extends Joinable> consumer) {
    ...
    public static Collection<JoinableMap> evaluate(Queryable queryable) {
        return queryable.evaluate(new ReadableDataSource());
    }
    ...
    public static Query compile(String queryDefinition) {
        return compile(queryDefinition, SystemProperties.get(LayerSystemProperties.Query.DEFAULT_COMPILER));
    }
